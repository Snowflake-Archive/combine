  ...
  >
    <div class="bg-slate-800 p-6 rounded-xl">
      ...
      <div class="mt-1">
        ...
        <input
          placeholder="URL"
          class="bg-slate-700 px-4 py-1 rounded-xl w-full"
          bind:value={mainURL}
        />
      </div>
      <div class="mt-1">
        ...
        <input
          placeholder="URL"
          class="bg-slate-700 px-4 py-1 rounded-xl w-full"
          bind:value={tortiseURL}
        />
      </div>
      <p class="mt-1 max-w-72 leading-4 text-xs">
        Upon clicking "Send", this turtle will receive the update file sent. If
        the update fails, you will need to install the update manually.
      </p>
      <p class="mt-1 max-w-72 leading-4 text-xs">
        This turtle will also disconnect momentarily while it installs the
        update.
      </p>
      <div class="flex flex-row gap-2 mt-3">
        <button
          class="px-3 py-1 bg-green-500 rounded-full pt-[0.125rem] hover:bg-green-400"
          on:click={sendUpdate}
        >
          Send
        </button>
        <button
          class="px-3 py-1 bg-red-500 rounded-full pt-[0.125rem] hover:bg-red-400"
          on:click={() => (updateScreenVisible = false)}
        >
          Cancel
        </button>
      </div>
    ...
    <div class="flex flex-row gap-8 mt-4 max-xl:flex-col">
      <div class="flex flex-col gap-4 max-w-52 max-xl:max-w-full">
        ...
        <div class="flex flex-row gap-2">
          <button
            class={`px-3 py-1  rounded-full pt-[0.125rem] ${currentTurtle?.state.paused ? "hover:bg-green-400 bg-green-500" : "hover:bg-orange-400 bg-orange-500"}`}
        ...
        {#if currentTurtle.config?.fuelLimit !== "unlimited" && currentTurtle.config?.fuelLimit}
          <div>
            ...
            <div class="text-sm leading-3">
              {currentTurtle.state.fuel} / {currentTurtle.config.fuelLimit}
            </div>
          </div>
        {/if}

        <InventoryComponent
          inventory={currentTurtle.inventory}
          updateIter={invUpdateIter}
          {config}
        />
        <div class="grid grid-cols-2">
          <div>
            <header class="uppercase font-bold text-xs leading-4">
              Version
            </header>
        ...
        <button
          class="px-3 py-1 bg-blue-500 rounded-full pt-[0.125rem] hover:bg-blue-400"
          on:click={() => (updateScreenVisible = true)}
        >
          Send Update...
        </button>
      </div>

      <Map turtle={currentTurtle} {updateIter} {config} />
    </div>
    <Chart {currentTurtle} {updateIter} />
  </main>
{:else}
