...
<main class="bg-slate-900 min-h-screen flex flex-row">
  {#if isAuthed && config}
    <main class="w-full">
      <nav
        class={`h-screen max-xl:w-full w-64 bg-slate-800 border-r-slate-950 border-r-[1px] border-solid py-4 flex flex-col justify-between fixed max-xl:py-2 ${menuVisible ? "" : "max-xl:h-11"} overflow-hidden transition-all duration-500 z-50`}
      >
        <div class="flex flex-col">
          <div
            class="text-lg px-4 font-bold pb-2 flex flex-row justify-between items-center"
          >
            {config?.name || "Snowflake Combine"}
            <button
              class="max-xl:visible invisible"
              on:click={() => {
          ...
          {#each [...allStates] as turtle}
            ...
            <div
              class={`flex items-center px-4 py-1 hover:bg-slate-700 ${currentTurtle?.id == turtle.id ? "bg-slate-600" : ""} cursor-pointer`}
              on:click={() => {
                menuVisible = false;
            ...
            >
              ...
              {/if}
              <img
                src={config.cropIcons[turtle.block] ||
                  "/icons/minecraft/textures/item/diamond_hoe.png"}
                class="w-6 h-6"
                style="image-rendering: crisp-edges;"
                alt="turtle"
              />
              <div
                class="pl-2 h-6 text-nowrap overflow-hidden whitespace-nowrap text-ellipsis"
              >
                {turtle.name}
              </div>
            </div>
        ...
        <div class="px-2">
          ...
          <div class="opacity-50 leading-3 text-xs">v2.1</div>
      ...
      <main
        class={`p-4 w-full max-xl:p-2 max-xl:pl-2 max-xl:pt-[2.25rem] pl-[18rem]`}
      >
        {#if currentTurtle}
          <CropFarm
            {currentTurtle}
  ...
  {:else}
    <main class="flex flex-col justify-between h-screen w-screen">
      ...
      <div class="flex items-center justify-center flex-col gap-2">
        ...
        <input
          type="password"
          class="bg-slate-600 border-slate-500 border rounded-lg p-1 px-2"
          bind:value={key}
        />
        <label class="flex items-center gap-2">
          <input
            type="checkbox"
            class="border-slate-500 border rounded-lg p-1 px-2"
            bind:checked={remember}
          />
          <span class="white">Remember me</span>
        </label>
        <button
          class="bg-slate-600 border-slate-500 border rounded-lg p-1 px-2 hover:border-slate-400 transition-colors"
          on:click={auth}>Log In</button
        >
        {#if loginMessage}
